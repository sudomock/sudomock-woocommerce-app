...
$icons = array(
    ...
    'wand'    => '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 4-1 1 4 4 1-1a2.83 2.83 0 1 0-4-4z"/><path d="m13 6-8.5 8.5a2.12 2.12 0 1 0 3 3L16 9"/></svg>',
...
$icon_html = '';
if ( ! empty( $a['showIcon'] ) && isset( $icons[ $a['iconStyle'] ] ) ) {
    $icon_html = $icons[ $a['iconStyle'] ];
}
...
$btn_style = sprintf(
    ...
    . '%s',
    ! empty( $a['fullWidth'] ) ? '100%' : 'auto',
    $a['paddingY'], $a['paddingX'],
    $a['fontSize'], $a['fontWeight'],
    $a['borderWidth'], esc_attr( $a['borderColor'] ),
    $a['borderRadius'],
    esc_attr( $a['bgColor'] ), esc_attr( $a['textColor'] ),
    $a['textTransform'],
    ! empty( $a['shadow'] ) ? 'box-shadow:0 2px 8px rgba(0,0,0,0.12);' : ''
);
...
<div <?php echo $wrapper_attributes; ?>>
    ...
    >
        <?php if ( ! empty( $a['showIcon'] ) && 'left' === $a['iconPosition'] ) echo $icon_html; ?>
        <?php echo esc_html( $a['label'] ); ?>
        <?php if ( ! empty( $a['showIcon'] ) && 'right' === $a['iconPosition'] ) echo $icon_html; ?>
    </button>

    <?php if ( ! empty( $a['bottomText'] ) ) : ?>
        <p class="sudomock-bottom-text" style="font-size:11px;color:#6b7280;margin:6px 0 0;font-family:inherit;">
            <?php echo esc_html( $a['bottomText'] ); ?>
        </p>
    <?php endif; ?>
</div>
